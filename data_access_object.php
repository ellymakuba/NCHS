...
/**
  ...
  function getPatientActiveEncounterList(){
    try{
      $stmt=$this->conn->prepare("SELECT enc.*,pa.p_name FROM encounter enc
      INNER JOIN patient pa ON pa.patient_id=enc.patient_id
      WHERE enc.open=? AND enc.admit=? AND admitted=?
      ORDER BY enc.encounter_id DESC");
      $params=array(1,1,0);
      $stmt->execute($params);
      return $stmt->fetchALL();
  ...
  function getAdmissionByEncounterId($id){
    try{
      $stmt=$this->conn->prepare("SELECT ad.*,w.name as ward_name FROM admission ad
        INNER JOIN ward w ON ad.ward=w.id
        WHERE ad.encounter_id=?");
      $params=array($id);
      $stmt->execute($params);
      $stmt->setFetchMode(PDO::FETCH_ASSOC);
      return $stmt->fetch();
  ...
  function doctorAllowDischarge($id){
    try{
      $stmt=$this->conn->prepare("UPDATE encounter SET allow_discharge=? WHERE encounter_id=?");
      $params=array(1,$id);
      $stmt->execute($params);
    }
    catch(PDOException $e){
  ...
  function closePatientEncounter($encounterId){
    try{
      $stmt=$this->conn->prepare("UPDATE encounter SET open=? WHERE encounter_id=?");
      $params=array(0,$encounterId);
      $stmt->execute($params);
    }
    catch(PDOException $e){
      echo $e->getMessage();
  ...
  function dischargePatient($encounterId){
    try{
      $stmt=$this->conn->prepare("UPDATE admission SET discharged=? WHERE encounter_id=?");
      $params=array(1,$encounterId);
      $stmt->execute($params);
    }
    catch(PDOException $e){
